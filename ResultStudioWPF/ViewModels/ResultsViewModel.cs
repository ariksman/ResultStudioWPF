...
namespace ResultStudioWPF.ViewModels
{
    public class ResultsViewModel : ViewModelBase
    {
        ...
        private void SetUpModel()
        {
            ...
            var measurementValueAxisX = new LinearAxis() { MajorGridlineStyle = LineStyle.Solid, MinorGridlineStyle = LineStyle.Dot };

            var measurementNumberAxisY = new LinearAxis() { Position = AxisPosition.Bottom };
            var measurementValueAxisY = new LinearAxis() { MajorGridlineStyle = LineStyle.Solid, MinorGridlineStyle = LineStyle.Dot };

            var measurementNumberAxisZ = new LinearAxis() { Position = AxisPosition.Bottom };
            var measurementValueAxisZ = new LinearAxis() { MajorGridlineStyle = LineStyle.Solid, MinorGridlineStyle = LineStyle.Dot };
        ...
        private void CreateMeasurementDataForPlots()
        {
            foreach (var measurementPoint in _dataSet)
            {
                if (measurementPoint.AxisName == Constants.MeasurementAxis.X)
                {
                    ...
                    _referenceX.Add(new DataPoint(measurementPoint.MeasurementNumber, (new ViewModelLocator()).SettingsViewModel.XAxisReference));
                }
            }
            ...
            foreach (var measurementPoint in _dataSet)
            {
                if (measurementPoint.AxisName == Constants.MeasurementAxis.Y)
                {
                    ...
                    _referenceY.Add(new DataPoint(measurementPoint.MeasurementNumber, (new ViewModelLocator()).SettingsViewModel.YAxisReference));
                }
            }
            ...
            foreach (var measurementPoint in _dataSet)
            {
                if (measurementPoint.AxisName == Constants.MeasurementAxis.Z)
                {
                    ...
                    _referenceZ.Add(new DataPoint(measurementPoint.MeasurementNumber, (new ViewModelLocator()).SettingsViewModel.ZAxisReference));
                }
            }
        }
